...
<div style="width: fit-content; display: flex; align-items: center;">
    <a href="{{ route('doctor.employees.edit', $item) }}" class="text-primary p-1 mx-1 fs-6" title="@lang('translate.edit')">
    ...
    <form action="{{ route('doctor.employees.destroy', $item) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('translate.are_you_sure') }}');">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-link text-danger p-1 m-1 border-0" title="@lang('translate.delete')">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="icon">
