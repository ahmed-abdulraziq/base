...
@section('breadcrumbs', Breadcrumbs::render('clinic.appointments.create'))

@section('content')
    <div class="col-md-12">
        <x-forms.form route="dashboard.clinic.appointments.store" method="POST" submitText="{{ __('translate.save') }}" formClass="form-horizontal" :title="__('translate.create_appointment')">

            <x-forms.select name="patient_id" :label="__('translate.patient')" :options="$patients" required col="col-md-6" />
            <x-forms.select name="doctor_id" :label="__('translate.doctor')" :options="$doctors" required col="col-md-6" />
