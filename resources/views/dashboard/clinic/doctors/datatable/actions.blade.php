<div style="width: fit-content; display: flex; align-items: center;">
    @if (!$item->approved_at)
        <form action="{{ route('dashboard.clinic.doctors.approve', $item->id) }}" method="POST" class="d-inline">
            @csrf
            <button type="submit" class="btn btn-link text-success p-1 mx-1 fs-6" title="{{ __('translate.approve') }}">
    ...
    @endif
    <a href="{{ route('dashboard.clinic.doctors.edit', $item->id) }}" class="text-primary p-1 mx-1 fs-6">
    ...
    <a href="javascript:void(0)" role="button" class="text-danger p-1 m-1 delete-btn cursor-pointer" data-url="{{ route('dashboard.clinic.doctors.destroy', $item->id) }}">
