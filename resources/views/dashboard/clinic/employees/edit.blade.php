...
@section('content')
    <div class="col-md-12">
        <x-forms.form route="dashboard.clinic.employees.update" :model="$employee->employee_id" method="PUT" submitText="{{ __('translate.save') }}" formClass="form-horizontal" :title="__('translate.edit_employee')">

            <x-forms.input name="first_name" :label="__('translate.first_name')" required :value="$employee->first_name" col="col-md-6" />
            <x-forms.input name="last_name" :label="__('translate.last_name')" required :value="$employee->last_name" col="col-md-6" />
